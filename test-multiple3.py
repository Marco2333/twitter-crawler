...
class Crawler:
	def __init__(self):
		...
		sql = "select screen_name from standardusers"
		users = []
		cursor.execute(sql)
		...
		# cate = api[0].GetUserSuggestionCategories()
		# for i in cate:
		# 	print i

		# users = api[0].GetUserSuggestion(cate[0])
		# print len(users)
		...
		with open("c:\\relationships%d" % i,"a") as f:
			while i < 2124:
				...
				while j < len(users):
					...
					try:
						json =  api[api_index].ShowFriendship(source_screen_name= users[i], target_screen_name= users[j])
					except Exception as e:
						print e
					...
					if json['relationship']['source']['followed_by'] == True:
						relationships.append((users[i],users[j]))
						print (users[i],users[j])
					j += 1
					print j
